...
<form method="post" action="{{ $route }}" class="mt-6">
    @csrf
    <div class="mb-6">
        <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">{{ $form['principal']['label'] }}</label>
        <input type="text" name="{{ $form['principal']['name'] }}"
        ...
        />
        @error($form['principal']['name'], 'form')
        <p class="mt-2 text-sm text-red-600 dark:text-red-500"><span class="font-medium">{{ $message }}</span></p>
        @enderror
    </div>
    <div class="mb-6">
        <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">{{ $form['password']['label'] }}</label>
        <input type="password" name="{{ $form['password']['name'] }}"
               class="bg-white-50 border border-gray-300 text-white-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"/>
        @error($form['password']['name'], 'form')
            <p class="mt-2 text-sm text-red-600 dark:text-red-500"><span class="font-medium">{{ $message }}</span></p>
        @enderror
    </div>
    <button type="submit"
